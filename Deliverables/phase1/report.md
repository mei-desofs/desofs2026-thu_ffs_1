...
*Unregistered User*:
 ...
 - Can only apply to the Supplier Program to become a registered supplier.
...
*Administrator*:
 ...
 - Manages suppliers, and overall system configurations.
 - Oversees platform performance and administrative processes.

*Dietitian*:
 - Registered user responsible for planning weekly meals.
 - Creates, edits and publishes menus on the platform.
 - Ensures menus include nutritional information.
...
*Supplier*:
 - Registered user delivers the organic products according to the contract to the Central Canteen.
 - Can receive product orders outside the contract.
...
- REQ1.3: After 3 failed attempts. the user has to wait 5 minutes before attempting to log in again and receives an alert notification by email.
...
- REQ6.2: The system must calculate the amount of each product needed for the next week based on the meal planning and the reservations of the current week.
...
- Operations such as meal reservations, reservation cancellations, delivery validation and stock updates must be processed reliably.
- Data related to reservations, stock, deliveries must not be lost in case of partial failures.
- The database must preserve data integrity and ensure correct transactional behavior.
- The system must maintain logs of important events, such as authentication attempts, reservations, delivery acceptance or rejection and stock alerts.
- Historical data must be preserved for auditability, traceability, and reporting purposes.

#### 6. Usability and Accessibility

The system must be easy to use by everyone.
...
- The system must support integration with the External Portal for authentication and consumer-related information.
...
These are required activities during software development to ensure that the BioCantinas application does not contain vulnerabilities.
...
* **Business logic errors that could be exploited:** e.g., manipulating the Delivery Reception and Validation process, bypassing payment requirements for Meals, or unauthorized data sync with the **External Portal**.

#### 4. Static Application Security Testing (SAST)
* Use static code analysis tools (e.g., **SonarQube**) integrated directly into the **GitHub Actions** CI/CD pipelines to detect vulnerabilities early in the codebase.
...
* **Prevent secret exposure:** Strictly use **GitHub Secrets** to manage sensitive data such as JWT secret keys, MySQL connection strings, and **External Portal API keys/certificates**. Never hardcode these in the repository.

#### 6. Logging and Monitoring
* Implement secure logging of security-relevant events, such as User Authentication logins, unauthorized access attempts to Delivery History, or errors during communication with the **External Portal**.
...
* Implement **integration tests** to verify secure interactions between the .NET 8 backend, the MySQL database, and the **External Portal API**.
...
All of these will be managed as **NuGet packages**, API integrations, and GitHub Actions workflows, under the umbrella of the **.NET 8.0 SDK** and runtime. Proper version control, security audits (especially for the External Portal API connection), and update processes will be applied to maintain system stability and integrity.
